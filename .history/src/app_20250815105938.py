...
# --------- Config ---------
MODEL_NAME = os.getenv("MODEL_NAME", "BreastSeg")
...
def get_model() -> ModelWrapper:
    global _model
    if _model is None:
        _model = ModelWrapper(MODEL_NAME, device=DEVICE)
    return _model
...
async def predict(file: UploadFile = File(...), return_images: bool = True):
    ...
    mask = pred.squeeze().cpu().numpy().astype(np.float32)  # (H,W) {0,1}
    ...
    if return_images:
        gray01 = img_t.squeeze().cpu().numpy()  # (H,W) in [0,1]
...
demo = gr.Interface(
    ...
    description="FastAPI backend with Gradio UI. Model loaded from MLflow Registry.",
)
